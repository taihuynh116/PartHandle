...
namespace PartHandle
{
    class GeomUtil
    {
        ...
        public static bool IsSmallerInPlan(XYZ first, XYZ second)
        {
            if (IsEqual(first, second)) return true;
            XYZ f = new XYZ(first.X, first.Y, 0), s = new XYZ(second.X, second.Y, 0);
            if (IsEqual(f, s))
                return true;
            if (GetLength(f) < GetLength(s))
                return true;
            return false;
        }
        ...
        public static bool IsBiggerInPlan(XYZ first, XYZ second)
        {
            if (IsEqual(first, second)) return true;
            XYZ f = new XYZ(first.X, first.Y, 0), s = new XYZ(second.X, second.Y, 0);
            if (IsEqual(f, s))
    ...
    public class UserPlane
    {
        ...
        public UserPlane(XYZ origin, XYZ basisX, XYZ basisY)
        {
            if (!(GeomUtil.IsEqual(GeomUtil.GetLength(basisX), 1) && GeomUtil.IsEqual(GeomUtil.GetLength(basisY), 1)))
                throw new Exception("Two basises must be unit vector.");
            if (!GeomUtil.IsEqual(GeomUtil.DotMatrix(basisX, basisY), 0))
                throw new Exception("Two basises must perpepdicular each other.");
            this.Origin = origin; this.XVector = basisX; this.YVector = basisY;
            this.Normal = GeomUtil.CrossMatrix(XVector, YVector);
        }
        public static implicit operator Plane(UserPlane up)
        {
            return Plane.CreateByOriginAndBasis(up.Origin, up.XVector, up.YVector);
        }
    }
